...
void carSORT(list* arr, int left, int right) { //차량번호에 대한 퀵정렬 내림차순
    if (left < right)						   // 차량 번호는 랜덤으로 입력받기때문에 효율이 좋은 퀵정렬 사용
    {
        ...
        while (low <= high)
        {
            while (low <= right && arr[low].car_num >= arr[pivot].car_num)
                low++;
            while (high > left && arr[high].car_num <= arr[pivot].car_num)
                high--;
            //엇갈리면 high와 pivot값 교체
            if (low > high)
                SWAP(arr, high, pivot);
            //엇갈리지않으면 low와 high 교체
            else
                SWAP(arr, low, high);
        }
        //각각 정렬된 수의 전, 후 에서 똑같이 순환반복
        carSORT(arr, left, high - 1);
        carSORT(arr, high + 1, right);
    }
}
void carUPSORT(list* arr, int left, int right) { //차량번호에 대한 퀵정렬 오름차순
    if (left < right)
    {
        ...
        carUPSORT(arr, left, high - 1);
        carUPSORT(arr, high + 1, right);
    }
}
void cusSORT(list* arr, int left, int right) {//고객 번호에 대한 퀵정렬 내림차순
    if (left < right)                         //고객 번호는 랜덤으로 입력받은 차량번호를 정리하고 나서 무작위로 섞이기 때문에 퀵정렬 사용
    {
        ...
        while (low <= high)
        {
            while (low <= right && arr[low].cus_num >= arr[pivot].cus_num)
                low++;
            while (high > left && arr[high].cus_num <= arr[pivot].cus_num)
                high--;
        ...
        cusSORT(arr, left, high - 1);
        cusSORT(arr, high + 1, right);
    }
}
void cusUPSORT(list* arr, int left, int right) {//고객 번호에 대한 퀵정렬 오름차순
    if (left < right)
    {
        ...
        cusUPSORT(arr, left, high - 1);
        cusUPSORT(arr, high + 1, right);
    }
}
//==================퀵정렬 끝 ===========================

void list_print(list* arr, int a) {  // 입출차 리스트 값 출력
    printf("\t\t|\t고객 번호\t\t차량 번호       |\n");
